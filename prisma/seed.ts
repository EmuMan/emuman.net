...
const art = [
  {
    ...
    description:
      "A desert scene I made in Blender, with touch-ups in photoshop.",
  },
  {
  ...
  },
  {
    file: "gemstones.png",
    description:
      "An array of physically accurate gemstones inspired by Steven Universe, rendered using LuxCore.",
  },
  {
    file: "calendar.png",
    description:
      "A part of a strangely coherent dream I had, photorealistically rendered out.",
  },
  { file: "bound.png", description: '"Bound". Hollow Knight is a good game.' },
];
...
const originalSongs = [
  {
    ...
    songId: "sunset_cafe",
    description:
      "One of the first songs I really made, all the way back in 2018 or something. Aivi and Surasshu were large inspirations, sort of that neo-chiptune vibe (I don't know if that is a term but it is now).",
  },
  {
    index: 2,
    title: "Through an Endless Sky",
    songId: "through_an_endless_sky",
    description:
...
async function main() {
  ...
  await prisma.programmingProject.deleteMany();
  await prisma.game.deleteMany();
  await prisma.cornFeature.deleteMany();
  ...
  console.log(`Seeded ${art.length} art pieces`);
  ...
  console.log(`Seeded ${originalSongs.length} original songs`);

  // Seed Songtober Songs
  for (const song of songtoberSongs) {
    await prisma.song.create({
      data: {
        ...song,
        songId: `songtober_${song.songId}`,
        collection: "songtober_2020",
      },
    });
  }
  console.log(`Seeded ${songtoberSongs.length} songtober songs`);

  // Seed Programming Projects
  for (let i = 0; i < programmingProjects.length; i++) {
    await prisma.programmingProject.create({
      data: {
        ...programmingProjects[i],
        order: i,
      },
    });
  }
  console.log(`Seeded ${programmingProjects.length} programming projects`);
  ...
  // Seed Corn Features
  for (let i = 0; i < cornFeatures.length; i++) {
    await prisma.cornFeature.create({
      data: {
        ...cornFeatures[i],
        order: i,
      },
  ...
  console.log(`Seeded ${cornFeatures.length} corn features`);
